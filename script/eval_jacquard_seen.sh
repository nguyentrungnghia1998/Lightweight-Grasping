...
for file in "$folder_path"/$pattern; do
    if [ -f "$file" ]; then
        ...
        python evaluate.py --dataset jacquard --dataset-path data/jacquard_seen/0/ --iou-eval --use-depth 0 --seen 0 --split 0.01 --network "$file"  # Execute the command with the file as a parameter
    fi
done
